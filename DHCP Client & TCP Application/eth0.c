...
dhcpstatemachine statemachine = { .gatewayaddressvalue = { 0, 0, 0, 0 }, .serveraddressvalue = { 0, 0, 0, 0 },
                            .state = 0, .xid = 0, .clientipaddress = { 0, 0, 0, 0 },
                            .youripaddress = { 0, 0, 0, 0 }, .temp1 = 0, .temp2 = 0 };
...
uint32_t sum;
uint8_t macAddress[HW_ADD_LENGTH] = { 2, 3, 4, 5, 6, 133};
...
uint8_t Dhcpoption(uint8_t option[], uint8_t operation, uint8_t* data, uint8_t length,
                      uint8_t index)
{
    option[index] = operation;
    option[index + 1] = length;
    memcpy(&option[index + 2], data, length);
    return length + 2;
}

void sendDhcpMessage(uint8_t packet[], uint8_t type)
{
    uint8_t parameter[4] = { 1, 3, 6, 15 };
    uint8_t tmp_Client_Identifier[HW_ADD_LENGTH + 1] = { 1, 2, 3, 4, 5, 6, 133 };
    uint8_t tmp_Host_Identifier[10] = "DHCP Client";
    uint8_t required_ip_address[IP_ADD_LENGTH] = { 192, 168, 1, 133 };

    uint8_t end_option = 0;
    uint8_t option_length = 0;
    uint16_t length_of_udp;
    uint16_t length_of_dhcp;
    uint16_t tmp16;
    ...
    if (type == 1)
    {
        ...
        memcpy(&statemachine.serveraddressvalue, &ipAddress, 4);
        memcpy(&dhcp->yiaddr, &ipAddress, 4);
        memcpy(&statemachine.gatewayaddressvalue, &ipAddress, 4);
        memcpy(&dhcp->giaddr, &ipAddress, 4);
        memcpy(&dhcp->chaddr, &macAddress, 6);
        memset(dhcp->data, 0, 192);
        dhcp->xid = 0x34235345;
        statemachine.xid = 0x34235345;
        dhcp->magicCookie = 0x63538263;

        option_length = option_length + Dhcpoption(dhcp->options, 53, &type, sizeof(type), option_length);
        option_length = option_length + Dhcpoption(dhcp->options, 61,(uint8_t*) &tmp_Client_Identifier, sizeof(tmp_Client_Identifier), option_length);
        option_length = option_length + Dhcpoption(dhcp->options, 50, (uint8_t *) &required_ip_address, sizeof(required_ip_address), option_length);
        option_length = option_length + Dhcpoption(dhcp->options, 55, (uint8_t *) &parameter, sizeof(parameter), option_length);
        option_length = option_length + Dhcpoption(dhcp->options, 255, &end_option, sizeof(end_option), option_length);

        udp->sourcePort = htons(68);
        udp->destPort = htons(67);
        length_of_dhcp = sizeof(dhcpFrame) + option_length - 2;
        length_of_udp = length_of_dhcp + 8;
        udp->length = htons(length_of_udp);
        ...
        ip->length = htons(((ip->revSize & 0xF) * 4) + 8 + length_of_dhcp);
        sum = 0;
        ...
        etherSumWords(&udp->length, 2);
        etherSumWords(udp, 6);
        etherSumWords(&udp->data, length_of_dhcp);
        udp->check = getEtherChecksum();
    }
    else if (type == 3)
    {
        memcpy(&required_ip_address, &statemachine.youripaddress, 4);
        memset(dhcp, 0, sizeof(dhcpFrame));
        ...
        option_length = option_length + Dhcpoption(dhcp->options, 50, (uint8_t *) &required_ip_address,  sizeof(required_ip_address), option_length);
        option_length = option_length + Dhcpoption(dhcp->options, 55, (uint8_t *) &parameter, sizeof(parameter), option_length);
        option_length = option_length + Dhcpoption(dhcp->options, 54, (uint8_t *) &statemachine.serveraddressvalue, sizeof(statemachine.serveraddressvalue), option_length);
        option_length = option_length + Dhcpoption(dhcp->options, 12,(uint8_t *) &tmp_Host_Identifier,sizeof(tmp_Host_Identifier), option_length);
        option_length = option_length + Dhcpoption(dhcp->options, 255, &end_option, sizeof(end_option), option_length);
    ...
    etherPutPacket(ether, 14 + ((ip->revSize & 0xF) * 4) + 8 + length_of_dhcp);
}
